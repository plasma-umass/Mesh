...
static inline uint64_t ATTRIBUTE_ALWAYS_INLINE rotl(const uint64_t x, int k) {
  return (x << k) | (x >> (64 - k));
}

class Xoroshiro {
private:
  DISALLOW_COPY_AND_ASSIGN(Xoroshiro);

public:
  explicit Xoroshiro(uint64_t seed1, uint64_t seed2) {
    s[0] = seed1;
    s[1] = seed2;
  }
