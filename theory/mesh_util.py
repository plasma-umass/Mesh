...
def formatStrings(strings):
    ...
    for string in strings:
        ...
        new_strings.append((string, long(string, base=2), string.count("1"), False))
    return new_strings
...
class Splitter(object):
    """encapsulates splitting behavior for a trial. keeps track of multiple different
    splitting strings and can automatically cycle through them if required."""
...
#        print 'found strings'
    ...
    if bucket1 == None or bucket2 == None:
        raise Exception('must pass either buckets or string set to traverse function')
...
#                    matched.append(i)
#                    matched.append(j)
                    strings[i] = (strings[i][0], strings[i][1], strings[i][2], True)
                    strings[j] = (strings[j][0], strings[j][1], strings[j][2], True)
...
def greedyTraverse(meshes, bucket1 = None, bucket2 = None, strings = None, cutoff = None):
    """Looks for meshable pairs between the buckets greedily (looks first at 
    all potential meshes with the first string in bucket1 and anything in bucket
    2, then the second string in bucket 2 with everything in bucket 2, etc.
    adds found pairs to meshes in place. returns whether or not all strings 
    have been meshed."""
    #if only one string list is supplied, search it exhaustively for pairs using a simpler function
    if strings != None:
...
#                print "doing a break!"
            ...
            if not bucket1[i][3] and not bucket2[j][3]:
                ...
                num2 = bucket2[j][1]
                if num1 & num2 == 0:
                    bucket1[i] = (bucket1[i][0], bucket1[i][1], bucket1[i][2], True)
                    bucket2[j] = (bucket2[j][0], bucket2[j][1], bucket2[j][2], True)
